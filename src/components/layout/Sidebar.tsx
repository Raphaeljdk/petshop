...
interface NavItem { id: TabId; label: string; icon: React.ComponentType<{ className?: string }>; badge?: number }
  ...
  return <>
      ...
      {groups.map(group => <div key={group.label}>
        ...
        <div className="space-y-1">
          {group.items.map(item => {
            ...
            return <button key={item.id} type="button" aria-current={active === item.id ? 'page' : undefined}
              onClick={() => { onChange(item.id); onNavigate?.() }}
              className={cn('sidebar-nav-item w-full flex items-center gap-3 px-3 py-3 rounded-xl text-sm font-medium min-h-11', active === item.id ? 'text-slate-800' : 'text-white/85')}>
              <Icon className="size-[18px] shrink-0" />
              <span className="flex-1 text-left">{item.label}</span>
              {!!item.badge && item.badge > 0 && <Badge className="bg-orange-100 text-orange-900 text-xs h-5 min-w-5 px-1.5 justify-center">{item.badge}</Badge>}
            </button>
          })}
        </div>
      </div>)}
    ...
    <div className="p-4 border-t border-white/10">
      <div className="rounded-xl bg-white/5 border border-white/10 p-3 flex gap-3 items-center">
        ...
        <div><p className="text-sm font-semibold text-white">Nossa matilha</p><p className="text-xs text-white/60">Cuidado em cada detalhe.</p></div>
      </div>
    </div>
  </>
}
...
export function Sidebar({ active, onChange, counts, mobileOpen = false, onMobileClose }: SidebarProps) {
  ...
  return <>
    ...
    <Sheet open={mobileOpen} onOpenChange={open => { if (!open) onMobileClose?.() }}>
      <SheetContent side="left" className="sidebar-surface w-72 max-w-[88vw] p-0 gap-0 border-0 [&>button]:text-white"
        onCloseAutoFocus={event => { event.preventDefault(); document.getElementById('admin-menu-trigger')?.focus() }}>
