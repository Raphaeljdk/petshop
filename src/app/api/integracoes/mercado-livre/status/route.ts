...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({ configured: Boolean(process.env.MERCADO_LIVRE_CLIENT_ID && process.env.MERCADO_LIVRE_CLIENT_SECRET && /^[a-f\d]{64}$/i.test(process.env.MERCADO_LIVRE_TOKEN_ENCRYPTION_KEY || '')),
    ...
    return NextResponse.json({ error: 'Estado da integração indisponível. Verifique a migração do banco.' }, { status: 503 })
  }
}
